...
-- Function test : moc_from_ascii_ivoa
...
-- Creation of a BMOC
SELECT create_bmoc_psql(13, ARRAY[8202, 8203, 8206, 8218]) & create_bmoc_psql(13, ARRAY[8202, 8203, 8224, 8225]) AS intersection;
...
-- Query inside is_in_cone_psql
SELECT * FROM hip_table
WHERE
(
...
-- Query that needs skyregions'function contains
SELECT * FROM hip_table
WHERE
(
    hpx_hash_range(29, raicrs, deicrs)
...
(
    ...
    AND
    (
        skyregion_cone_contains(0.01814144, 3.94648893, 5.64323, raicrs, deicrs)
    )
);
